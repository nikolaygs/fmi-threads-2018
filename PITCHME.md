...
Стандартни нишки – те изпълняват задачи, които са свързани с основната идея на програмата. Всяка JVM работи, докато има поне една стандартна нишка, която още не е приключила с изпълнение на своята задача

@fa[arrow-down]
+++

Демонов – те изпълняват задачи, които не са част от логиката на програмата, а по-скоро се класифицират като общи услуги. JVM ще прекрати работата на нишките от този тип, без значение от състоянието в което се намират, ако няма поне една работеща стандартна нишка
...
```
  public void depositMoney(BankAccount acc, double amount) {
    ...
    // Критична секция – една-единствена нишка за дадена сметка acc може //
    // изпълнява кода в синхронизираната секция
    synchronized (acc) {
      acc.deposit(amount);
    }

    // Не-критична секция - много нишки могат да бъдат тук едновремено
...
- По-този начин енкапсулираме монитора, по който синхронизираме вътре в самата имплементация, но използваме допълнителна памет
- Даден клас може да има няколко различни променливи, които да се ползват за монитори за охраняване на различно състояние
...
---
### Thread-safe обекти
Има нужда да синхронизираме само обекти, които са достъпвани от много нишки и могат да бъдат променяни.
Някой обекти са thread-safe по подразбиране:
- Локални примитивни променливи и локални референции, чийто обект не е споделен с друга нишка
- Stateless обекти
- Immutable обекти
- Обекти, които са ефективно final

---

### Volatile променливи

Всеки JVM гарантира, че простите операции върху примитиви с размер до 32 бита ще бъдат третирани като неделими. Това изключва променливите от тип long и double

@fa[arrow-down]
+++


Четенето и писането на 64 бита може да бъде разделено на 2 операции по 32 бита, което създава предпоставки за неконсистентност на данните:
...
Дефинирането на променлива като volatile има следните преимущества:
- Атомарност – гаранция, че простите операции върху 64-битови примитиви са неделими
- Видимост – стойността на променливата не се кешира в процесора, а се чете и съхранява директно в паметта. 

@fa[arrow-down]
+++

Ключовата дума volatile осигурява атомарност само и единствено на простите операции върху примитиви. За създаване на съставни неделими операции трябва да се използват синхронизирани блокове
